---
title: "Core Concepts And Code for Survival Prediction Modeling"
author: "Mark Zobeck"
date: 2024-12-04
...
This post provides a general workflow for developing a survival
prediction model using both frequentist and Bayesian methods. I wanted
to gather the code in one place for my reference and to make it easier
to compare and contrast the two approaches.

The analysis is based off of Frank Harrell's "[Case Study in Cox
Regression](https://hbiostat.org/rmsc/coxcase)" from Chapter 21 of his
wonderful book *Regression Modeling Strategies*.

We're going to produce survival predictions using a Cox proportional
hazards model: $$h(t|X) = h_0(t)\exp(X\beta)$$\
Where $h$ is the hazard function (instantaneous risk of event), $t$ is
time, $X$ is a vector of predictor variables (like age, treatment,
etc.), and $\beta$ is a vector of regression coefficients measuring the
impact of each predictor that will be estimated from the data.

We can express the outcome in terms of survival as
$$S(t | X) = S_{0}(t)^{\exp(X\beta)}$$ Where $S$ is a function to the
probability of survival beyond time $t$ given predictors $X$.
...
::: callout-note
For binary or survival (time-to-event) outcomes, there are three
criteria:\
i) small overfitting defined by an expected shrinkage of predictor
effects by 10% or less,\
ii) small absolute difference of 0.05 in the model's apparent and
adjusted Nagelkerke's R-squared value, and\
iii) precise estimation (within +/- 0.05) of the average outcome risk in
the population for a key timepoint of interest for prediction.
:::
...
pmsampsize(
  type = "s",      
### NEED TO FIGURE OUT R2 types blehhhh
  nagrsquared = 0.2,
  parameters = 36,  
  timepoint = 2,  
  meanfup = 3,    
...
This analysis examines survival data from a clinical trial investigating
estrogen treatment for prostate cancer, where the primary endpoint is
time to death from all causes.

Patients were right censored at 24 months if they were still alive at
...
```{r}
 # for print, summary, anova, validate
...
imp <- aregImpute(~ sz + sg + ap + sbp + dbp + age +
                   wt + hg + ekg + pf + bm + hx,
                  data = prostate, n.impute = 10, pr = F)

val <- function(fit){
  list(validate  = validate (fit, B=100),
       calibrate = calibrate(fit, B=100, u=5*12) )
}
attach(prostate)
...
f <- fit.mult.impute(S ~ rx + rcs(age, 4) + rcs(wt, 4) + pf + hx +
         rcs(sbp, 4) + rcs(dbp, 4) + ekg + rcs(hg, 4) +
         rcs(sg, 4) + rcs(sz, 4) + rcs(log(ap), 4) + bm,
         cph, imp, data = prostate,
         fun=val, fitargs=list(x=TRUE, y=TRUE, surv=TRUE, time.inc = 5*12), pr = F)
  
print(f, coefs=FALSE)
validate(f)
plot(calibrate(f, u = 5*12))

f2 <- cph(S ~ rx + rcs(age, 4) + rcs(wt, 4) + pf + hx +
         rcs(sbp, 4) + rcs(dbp, 4) + ekg + rcs(hg, 4) +
         rcs(sg, 4) + rcs(sz, 4) + rcs(log(ap), 4) + bm, 
         data = prostate, x=TRUE, y=TRUE, surv=TRUE, time.inc = 5*12)

f2
validate(f2)
plot(calibrate(f2, u = 5*12))
```
...
-   Fit `f.short` has same LR $\chi^2$ of 118 as the fit `f`, but with
    falsely low d.f.
-   All $\beta=1$
...
-   More accurate predictions could be obtained using stratification or
    time dep. cov.
