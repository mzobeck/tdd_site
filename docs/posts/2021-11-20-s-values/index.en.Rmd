...
subtitle: "Calibrate your intuition about p-values"
author: "Mark Zobeck"
date: 2021-11-20
...
svalue <- function(x,y) round(-log2(x), y)
...
Mathematically an s-value is defined as $s = -log_2(p)$ (the mathematical details of the development of this transformation are outside the scope of this post, but you can see @cole2021 for a more robust discussion). The result of this transformation can be interpreted as saying, "if I were to flip a fair coin ***s*** times, I would expect to get heads on all flips with probability ***p***." For example, if I flip a fair coin and get heads 4 times in a row, that corresponds to a probability of getting that outcome of `r round(2^(-4),2)` . Another way to phrase this: if I did 100 trials of 4 coin flips, only `r round(2^(-4),2)*100` would result in 4 heads. This transformation also works if we start with probability. If I tell you that I saw someone flip a coin and get heads each time and that the probability of this outcome using a fair coin was only around 0.01, you could plug it into the equation tell me that probability corresponds to roughly `r round(-log2(0.01), 0)` heads in a row (with rounding because fraction of coin flips are hard to visualize).
...
```{r, fig.align = 'center'}
s.tibble <- tibble(
  `p-value` = seq(0.001, 1, by = 0.001)
) %>% 
  mutate(`s-value` = -log2(`p-value`))

p <- ggplot(s.tibble, aes(`p-value`, `s-value`)) +
  geom_line() +
  scale_x_reverse(expand = expansion(mult = c(0, 0.05))) + 
  scale_y_continuous(breaks = 1:10,
                     expand = expansion(mult = c(0, 0.05))) +
  geom_vline(xintercept = 0.05, color = "cornflowerblue") +
  labs(x = "\nP-value", y = "S-value\n") 
...
```{r, fig.align = 'center'}
p2 <- ggplot(s.tibble, aes(`p-value`, `s-value`)) +
  geom_line() +
  scale_x_reverse(limits = c(0.1,0),
                  expand = expansion(mult = c(0, 0.05))) + 
  scale_y_continuous(breaks = 1:10,
                     expand = expansion(mult = c(0, 0.05))) +
  geom_vline(xintercept = 0.05, color = "cornflowerblue")  +
  geom_hline(yintercept = -log2(0.05), color = "cornflowerblue") +
  labs(x = "\nP-value", y = "S-value\n") 

p2

#ggsave("cover_image.png", p2)
```
...
s.tibble.5 <- tibble(
  ...
  mutate(s= round(-log2(p), 2))

s.tibble.5 %>% 
  arrange(desc(p)) %>% 
  gt() %>% 
  cols_label(p = md("**P-Value**"), 
             s = md("**S-Value**")) 
```
...
Notice that as the p-values get smaller, the s-value increase starts to accelerate. Going from p=0.1 to p=0.01 (an absolute difference of 0.09) results in a `r svalue(0.01,1) - svalue(0.1,1)` increase in s-values. This is very different than in the range of p-values from 1 to 0.25 (absolute difference of 0.75) that resulted in an increase in s-values of 2. Let's make this trend clearer in the table below.

```{r}
s.tibble.disp <- tibble(
...
1.  S-values correspond to the psychologically familiar situation of the surprise that you feel when flipping a coin that you think is fair and getting heads a certain number of times in a row. This builds intuition of how p-values provide evidence against the null hypothesis.
2.  S-values demonstrate that the magnitude of the *absolute* difference in p-values do not correspond to the difference in s-values across the range of possible p-values.
3.  S-values increase slowly over the range of p-values that surround the traditional cutoff values of p=0.05, demonstrating that p-value cutoffs are largely arbitrary and values in this range provide similar feelings of surprise against a null hypothesis.
4.  The increase in S-values will accelerate as p-values become smaller, demonstrating that information against the hypothesis accumulates in small p-values, contrary to the naive intuition that small absolute differences on the probability scale when the values are small are not overly meaningful.

I hope this discussion is helpful to calibrate your intuition about what p-values signify. See these references below to learn more [@rafi2020a; @greenland2019a].
